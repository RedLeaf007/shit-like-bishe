...
interface HomeApi {
   ...
   /**智能推荐(按购买类型推荐)*/
    ...
    /**考试记录*/
    @GET("examrecord/groupby")
    fun examRecordGroup(): Call<BaseResultBean<PageResultBean<ExamrecordItemBean>>>

    /*删除考试记录*/
    @GET("examrecord/deleteRecords")
    fun deleteRecords(@Query("userid") userid: Long,@Query("paperid") paperid: Long): Call<BaseResultBean<PageResultBean<Any?>>>

    /**论坛首页*/
    @GET("forum/flist?parentid=0&isdone=开放&sort=isTop,topTime,addtime&order=desc,desc,desc")
    fun forumIndex(@Query("page") page: Int,@Query("title") title: String?=null,@Query("limit") limit: Int = 20): Call<BaseResultBean<PageResultBean<ForumItemBean>>>
    /**我的论坛*/
    @GET("forum/page?parentid=0&sort=addtime&order=desc")
    fun myForum(@Query("page") page: Int,@Query("title") title: String?=null,@Query("limit") limit: Int = 20): Call<BaseResultBean<PageResultBean<ForumItemBean>>>

    /*论坛详情*/
    @GET("forum/list/{id}")
    fun forumList(@Path("id") id: Long): Call<BaseResultBean<ForumItemBean>>
