...
import com.design.appproject.ui.yundongjilu.ListFilterDialog
...
import com.design.appproject.databinding.YundongjilucommonListLayoutBinding
import com.design.appproject.logic.repository.HomeRepository
import com.design.appproject.logic.viewmodel.yundongjilu.ListModel
import com.design.appproject.ui.yundongjilu.ListAdapter
 ...
 * 运动记录列表页
 */
@Route(path = CommonArouteApi.PATH_FRAGMENT_LIST_YUNDONGJILU)
class ListFragment : BaseBindingFragment<YundongjilucommonListLayoutBinding>() {
    ...
    override fun initEvent() {
        setBarTitle("运动记录",mHasBack)
        setBarColor("#FFFFFF","black")
        binding.apply {
            addBtn.isVisible = (mIsBack && Utils.isAuthBack("yundongjilu","新增"))|| Utils.isAuthFront("yundongjilu","新增")
            addBtn.setOnClickListener {
                ARouter.getInstance().build(CommonArouteApi.PATH_ACTIVITY_ADDORUPDATE_YUNDONGJILU).navigation()
            }
            if (mIsHome){
            ...
            mListAdapter.setOnItemChildClickListener { adapter, view, position ->
                if (view.id==R.id.edit_tv){
                  ...
                  XPopup.Builder(requireActivity()).asConfirm("提示","是否确认删除") {
                      HomeRepository.delete<Any>("yundongjilu",listOf(mListAdapter.data[position].id)).observeKt {
                        it.getOrNull()?.let {
                            "删除成功".showToast()
                            loadData(1)
                        }
            ...
            srv.setAdapter(mListAdapter.apply {
                ...
                setOnItemClickListener { adapter, view, position ->
                    ArouterUtils.startFragment(CommonArouteApi.PATH_FRAGMENT_DETAILS_YUNDONGJILU,map=mapOf("mId" to data[position].id,"mIsBack" to mIsBack))
                }
            })
        }
    }
    private fun YundongjilucommonListLayoutBinding.initSearch() {
    ...
    private fun loadData(page:Int,searchParams:MutableMap<String,String>?=null){
        ...
        mIsBack.yes {
            mListModel.page("yundongjilu", requestParams)
        }.otherwise {
            mListModel.list("yundongjilu", requestParams)
        }
    }
}