...
import com.design.appproject.databinding.YundongjiluFilterDialogLayoutBinding
...
class ListFilterDialog(context: Context) : DrawerPopupView(context) {
    ...
    lateinit var binding: YundongjiluFilterDialogLayoutBinding
    ...
    override fun onCreate() {
        ...
        binding.apply {
            ...
            "有氧,无氧,拉伸".split(",").forEachIndexed {index, s ->
                yundongleixingFbl.addView(createCustomItemView(index,s,yundongleixingFbl))
            }
            resetBtn.setOnClickListener { //重置
                yundongleixingFbl.children.forEach { it.isSelected=false }
                callBackListener?.invoke(false,null)
            }
            ensureBtn.setOnClickListener { //确定
                params.clear()
                val yundongleixingString = yundongleixingFbl.children.filter { it.isSelected }.map { it.toConversion<TextView>()?.text.toString() }.joinToString()
                yundongleixingString.isNotNullOrEmpty().yes {
                    params.put("yundongleixing",yundongleixingString)
                }
