        ...
        mHomeViewModel.newsListLiveData.observeKt {
            it.getOrNull()?.let {
                ...
                it.data.list.forEachIndexed { index, newsListBean ->
                    newsLl.addView(creatNewItemView(index+1,newsListBean))
                }
            }
        }
    }

    private fun creatNewItemView(index:Int,newsListBean: NewsItemBean) =LayoutInflater.from(context).inflate(R.layout.item_news_layout,binding.newsLl,false).apply{
        findViewById<TextView>(R.id.news_title_tv).text = newsListBean.title
        findViewById<TextView>(R.id.index_tv).text = if (index<10) "0"+index else index.toString()
