...
import com.design.appproject.ui.genzongjianyi.ListFilterDialog
...
import com.design.appproject.databinding.GenzongjianyicommonListLayoutBinding
import com.design.appproject.logic.repository.HomeRepository
import com.design.appproject.logic.viewmodel.genzongjianyi.ListModel
import com.design.appproject.ui.genzongjianyi.ListAdapter
 ...
 * 跟踪建议列表页
 */
@Route(path = CommonArouteApi.PATH_FRAGMENT_LIST_GENZONGJIANYI)
class ListFragment : BaseBindingFragment<GenzongjianyicommonListLayoutBinding>() {
    ...
    override fun initEvent() {
        setBarTitle("跟踪建议",mHasBack)
        setBarColor("#FFFFFF","black")
        binding.apply {
            addBtn.isVisible = (mIsBack && Utils.isAuthBack("genzongjianyi","新增"))|| Utils.isAuthFront("genzongjianyi","新增")
            addBtn.setOnClickListener {
                ARouter.getInstance().build(CommonArouteApi.PATH_ACTIVITY_ADDORUPDATE_GENZONGJIANYI).navigation()
            }
            if (mIsHome){
            ...
            mListAdapter.setOnItemChildClickListener { adapter, view, position ->
                if (view.id==R.id.edit_tv){
                  ...
                  XPopup.Builder(requireActivity()).asConfirm("提示","是否确认删除") {
                      HomeRepository.delete<Any>("genzongjianyi",listOf(mListAdapter.data[position].id)).observeKt {
                        it.getOrNull()?.let {
                            "删除成功".showToast()
                            loadData(1)
                        }
            ...
            srv.setAdapter(mListAdapter.apply {
                ...
                setOnItemClickListener { adapter, view, position ->
                    ArouterUtils.startFragment(CommonArouteApi.PATH_FRAGMENT_DETAILS_GENZONGJIANYI,map=mapOf("mId" to data[position].id,"mIsBack" to mIsBack))
                }
            })
        }
    }
    private fun GenzongjianyicommonListLayoutBinding.initSearch() {
    ...
    private fun loadData(page:Int,searchParams:MutableMap<String,String>?=null){
        ...
        mIsBack.yes {
            mListModel.page("genzongjianyi", requestParams)
        }.otherwise {
            mListModel.list("genzongjianyi", requestParams)
        }
    }
}