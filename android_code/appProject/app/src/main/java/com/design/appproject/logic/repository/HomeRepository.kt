...
object HomeRepository : BaseRepository() {
    ...
    fun examRecordGroup() = fireX {
        homeService.examRecordGroup().await()
    }
    fun deleteRecords(userId: Long,paperId:Long) = fireX {
        homeService.deleteRecords(userId,paperId).await()
    }
    fun forumIndex(page:Int,title:String?=null)= fireX {
        homeService.forumIndex(page,title).await()
    }
    fun myForum(page:Int,title:String?=null)= fireX {
        homeService.myForum(page,title).await()
    }

    fun forumList(id:Long)= fireX {
        homeService.forumList(id).await()
    }
